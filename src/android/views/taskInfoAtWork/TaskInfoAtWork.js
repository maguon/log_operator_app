    ...
    renderHeader() {
        ...
        return (<View>
            <View style={{ padding: 10, backgroundColor: '#f2f6f9', borderBottomWidth: 0.5, borderColor: '#ccc' }}>
                ...
                <View style={{ flexDirection: 'row', paddingTop: 10, justifyContent: 'space-between' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <Text style={{ paddingLeft: 5, color: '#8c989f', fontWeight: 'bold' }}>{taskInfo.city_start_name ? `${taskInfo.city_start_name}` : ''}</Text>
                        <MaterialCommunityIcons name='ray-start-arrow' size={20} style={{ paddingLeft: 5, color: '#8c989f' }} />
                        <Text style={{ paddingLeft: 5, color: '#8c989f', fontWeight: 'bold' }}>{taskInfo.city_name ? `${taskInfo.city_name}` : ''}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <MaterialCommunityIcons name='account' size={16} color='#aaa' />
                        <Text style={{ paddingLeft: 5, color: styleColor }}>{taskInfo.drive_name ? `${taskInfo.drive_name}` : ''}</Text>
                    </View>
                </View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 10, alignItems: 'center', borderBottomWidth: 1, borderColor: styleColor }}>
                ...
                <Text style={{ fontSize: 11 }}>实际运送：<Text style={{ color: styleColor }}>{taskInfo.real_count ? `${taskInfo.real_count}` : ''}</Text></Text>
            </View>
        </View>)
    }
