...
export const getDemageList = () => async (dispatch, getState) => {
    const { loginReducer: { data: { user: { uid } } }, searchDamageReducer: { data: { searchDamageForm } } } = getState()
    try {
        const url = `${base_host}/damage?${ObjectToUrl({ declareUserId: uid, start: 0, size: pageSize, ...searchDamageForm })}`
...
export const getDemageListMore = () => async (dispatch, getState) => {
    const {
        ...
        demageListReducer, searchDamageReducer: { data: { searchDamageForm } } } = getState()
    if (demageListReducer.getDemageListMore.isResultStatus == 1) {
        ...
        if (!isComplete) {
            ...
            try {
                const url = `${base_host}/damage?${ObjectToUrl({ declareUserId: uid, start: demageList.length, size: pageSize, ...searchDamageForm })}`
                const res = await httpRequest.get(url)
                if (res.success) {
                    dispatch({
