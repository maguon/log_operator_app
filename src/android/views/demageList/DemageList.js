...
import * as searchDamageAction from '../searchDamage/SearchDamageAction'
        ...
        <TouchableOpacity style={styles.listItemContainer} onPress={() => {
            ...
            <View style={styles.listItemTopContainer}>
                <View style={styles.itemGroup}>
                    ...
                    <Text style={[globalStyles.smallText, styles.text]}>申报时间：{created_on ? `${moment(created_on).format('YYYY-MM-DD HH:mm')}` : ''}</Text>
                </View>
                {damage_status == 1 && <Text style={[globalStyles.smallText, { color: 'red' }]}>待处理</Text>}
                {damage_status == 2 && <Text style={[globalStyles.smallText, globalStyles.styleColor]}>处理中</Text>}
                {damage_status == 3 && <Text style={globalStyles.smallText}>已处理</Text>}
            </View>
            <View style={styles.listItemBodyContainer}>
                <View style={styles.itemGroup}>
                    <Text style={[globalStyles.midText, globalStyles.styleColor]}>VIN：{vin ? `${vin}` : ''}</Text>
                </View>
...
class DemageList extends Component {
    ...
    componentWillUnmount() {
       this.props.cleanSearchDamageForm()
    }
...
const mapDispatchToProps = (dispatch) => ({
    ...
    cleanSearchDamageForm: () => {
        dispatch(searchDamageAction.cleanSearchDamageForm())
    }
})
