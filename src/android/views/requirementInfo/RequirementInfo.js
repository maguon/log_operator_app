...
import * as requirementInfoAction from './RequirementInfoAction'
        ...
        return (<View style={{ flex: 1 }}>
            <View style={{ padding: 10, backgroundColor: '#f2f6f9', borderBottomWidth: 0.5, borderColor: '#ccc' }}>
                <View style={{ flexDirection: 'row', paddingTop: 10, justifyContent: 'space-between' }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        {requirementInfo.demand_status == 0 && <Text style={{ paddingLeft: 5, color: styleColor }}>已取消</Text>}
                        {requirementInfo.demand_status == 1 && <Text style={{ paddingLeft: 5, color: styleColor }}>未完成</Text>}
                        {requirementInfo.demand_status == 2 && <Text style={{ paddingLeft: 5, color: styleColor }}>完成</Text>}
                    </View>
                </View>
        ...
        return (
            <TouchableOpacity key={index} style={{ borderColor: '#eee', borderWidth: 1, marginHorizontal: 10, marginTop: 10 }}
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', backgroundColor: '#f3f6f8', padding: 5 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <MaterialCommunityIcons name='format-list-bulleted' size={20} color={styleColor} />
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        {item.load_task_status == 3 && <Text style={{ fontSize: 11, color: styleColor }}>已装车</Text>}
                        {item.load_task_status == 7 && <Text style={{ fontSize: 11, color: styleColor }}>已到达</Text>}
                        {item.load_task_status == 9 && <Text style={{ fontSize: 11, color: styleColor }}>已完成</Text>}
                    </View>
                </View>
                <View style={{ padding: 5 }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 5 }}>
                        <Text style={{ fontSize: 11 }}>计划运送：<Text style={{ color: styleColor }}>{item.plan_count ? `${item.plan_count}` : ''}</Text></Text>
                        <Text style={{ fontSize: 11 }}>实际运送：<Text style={{ color: styleColor }}>{item.real_count ? `${item.real_count}` : ''}</Text></Text>
                    </View>
                </View>
            </TouchableOpacity>
        )
